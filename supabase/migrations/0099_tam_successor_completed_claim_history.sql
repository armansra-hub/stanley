-- Published claims retain actor/token/lease history for exact idempotent
-- publication readback (0043). They are completed work, not active admissions.
-- Keep that history untouched; reading/final and nonfinal residual claims still
-- block. All exact source, provenance, membership and seed checks remain intact.
begin;
