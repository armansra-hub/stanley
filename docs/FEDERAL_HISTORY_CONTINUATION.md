...
Historical weak methods, including `name_only`, are selected individually. A fresh exact-identifier award detail plus current account identity can strengthen a binding; a current explicitly sourced related binding can reclassify that one weak direct relationship. The SQL transaction stores immutable before/after match images, retains all entity/award/transaction history, and uses an exact before-image CAS under the company lease. Reclassification preserves and zeroes only exact award-backed triggers (including contract timing) and account contract-metric triggers that need recomputation. Detail suppresses stale metric snapshots until the existing collector recomputes them. Insufficient evidence records `needs_evidence` and leaves the old link unchanged, revisiting after seven days. No bulk demotion of the latest200 audited weak-method links occurs. No TAM grade, membership, or original federal-discovery fence is changed.

Dedicated agent-authenticated `/api/agent/intelligence/federal-repair` provides a read-only GET snapshot and a finite POST with limit1–20. The finite worker uses the same company leases and atomic repair transaction, skips discovery/paid inference, and stops at its220-second budget. GET reports current weak IDs, eligible outstanding work, active leases, unresolved evidence and recent outcomes so a timeout is inspected before another run. Zero eligible work does not imply all historical links were corrected. Ordinary identity enrollment excludes empty accounts without usable current identity observations or weak matches; two of every three five-minute scheduling windows prefer due recipient continuations, with the third favoring new evidence.

Migration0095 replaces capped client-side match/receipt scans with the next exact eligible weak match selected from the complete database set under that company lease. Companies with more than100 verified recipients continue normally. Recent receipts exclude already attempted exact matches, and the has-more flag counts only untreated eligible work. A stale before-image CAS keeps work pending for prompt reselection. A500-binding fixture covers selection beyond200 strong and200 recently receipted links, receipt-driven advancement, the last candidate and unresolved-but-receipted completion.
