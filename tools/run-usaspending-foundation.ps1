    ...
    if ($Mode -eq 'Awards' -and $BatchSize -eq 1) {
      while ([int]$result.retryRemaining -gt 0) {
        ...
        Write-Receipt ([ordered]@{
          ...
          status = $chunkReceipt.status; awardDone = $chunkReceipt.awardDone
          retryRemaining = $chunk.retryRemaining
          retryDeadLettered = @($chunk.retryDeadLettered)
          recoveryBlocked = [bool]$chunk.recoveryBlocked
          receiptErrors = @($chunk.receipts | Where-Object { $_.error } | ForEach-Object { $_.error })
        })
        $result = $chunk
        if (@($chunk.retryDeadLettered).Count -gt 0) { break }
      }
    }
  }
  catch {
