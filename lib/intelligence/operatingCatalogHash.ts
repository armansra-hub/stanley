/** Synchronous SHA-256 for small, immutable client-safe catalog contracts.
 * This is a content identity, not credential/security handling. */
export function catalogSha256(text: string): string {
  const source = new TextEncoder().encode(text);
  const padded = new Uint8Array(Math.ceil((source.length + 9) / 64) * 64);
  padded.set(source); padded[source.length] = 0x80;
  const view = new DataView(padded.buffer);
  view.setUint32(padded.length - 8, Math.floor(source.length / 0x20000000));
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let i = 0; i < 64; i++) {
      ...
      const first = (h + upper1 + choose + constants[i] + words[i]) >>> 0;
