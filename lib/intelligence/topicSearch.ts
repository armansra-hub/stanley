...
export function buildTopicSearchResult(raw: TopicSearchRaw) {
  ...
  const accounts = (selected.length ? raw.accounts : []).flatMap(account => {
    ...
    if (mode === "all" && matches.some(topic => topic.state !== "supported")) return [];
    const topics = matches.filter(topic => topic.state === "supported");
  ...
  return { enabled: raw.enabled, topics: selected, mode, topicCounts: raw.topicCounts ?? {}, accounts, hasMore: raw.hasMore, nextCursor: raw.nextCursor,
