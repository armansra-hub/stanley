...
function row(id: string) { return { "Sub-Award ID": id, "Sub-Awardee Name": "Acme", "Sub-Award Date": "2026-09-01", "Sub-Award Amount": 100, primeAwardId: "P1" }; }
...
beforeEach(() => {
  vi.clearAllMocks();
  links = [{ government_entity_id: ENTITY, government_entities: { legal_name: "Acme", dba_name: null } }];
...
describe("resumable exact-company subaward history", () => {
  it("finishes empty aliases and metrics in one invocation without creating retry debt", async () => {
    links[0].government_entities = { legal_name: "Acme Legal", dba_name: "Acme DBA" };
