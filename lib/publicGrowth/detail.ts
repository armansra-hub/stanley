...
export async function getPublicGrowthDetail(companyId: string): Promise<PublicGrowthDetail> {
  ...
  const [{ data: matches }, { data: metrics }, { data: headcount, error: headcountError }, { data: revenue }, { data: opportunityMatches }, { data: company, error: companyError }] = await Promise.all([
  ...
  return { entities, contractMetrics: metrics ?? null, contractRevenueByYear, awards, naicsSize, headcount: visibleHeadcount, revenue: revenue ?? [], opportunities: opportunityMatches ?? [] };
}
