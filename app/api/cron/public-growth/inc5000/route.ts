...
import { ingestInc5000Matches, type Inc5000MatchInput } from "@/lib/publicGrowth/inc5000";
...
function authorized(req: NextRequest) {
  ...
  return Boolean(supplied && ((process.env.TAM_GROWTH_SWEEP_SECRET && supplied === process.env.TAM_GROWTH_SWEEP_SECRET)
...
export async function POST(req: NextRequest) {
  if (!authorized(req)) return NextResponse.json({ error: "unauthorized" }, { status: 401 });
  const body = await req.json() as { matches?: Inc5000MatchInput[] };
  const rows = Array.isArray(body.matches) ? body.matches.slice(0, 250) : [];
  if (!rows.length) return NextResponse.json({ error: "matches required" }, { status: 400 });
  return NextResponse.json(await ingestInc5000Matches(rows));
}
