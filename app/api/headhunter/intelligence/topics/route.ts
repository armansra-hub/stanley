...
export async function GET(req: NextRequest) {
  ...
  if (!topics || (after && !isUuid(after)) || !Number.isInteger(limit) || limit < 1 || limit > 12) {
  ...
  try {
    const result = await withServiceDeadline(Date.now() + 20_000, async () => {
      const { data, error } = await serviceClient().rpc("intelligence_topic_search", { p_topics: topics, p_after: after || null, p_limit: limit });
