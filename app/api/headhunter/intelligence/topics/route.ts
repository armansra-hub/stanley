...
export async function GET(req: NextRequest) {
  ...
  try {
    const result = await withServiceDeadline(Date.now() + 20_000, async () => {
      const { data, error } = await serviceClient().rpc(visibility === "explore" ? "intelligence_topic_explore" : "intelligence_topic_search", { p_topics: topics, p_after: after || null, p_limit: limit, p_mode: mode, p_show_hidden: showHidden });
